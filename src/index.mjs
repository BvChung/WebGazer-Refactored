...
const regressionMap = {
	...
	ridgeWeighted: function () {
		return new RidgeWeightedReg();
	},
	ridgeThreaded: function () {
		return new RidgeRegThreaded();
	},
};
 ...
 */
const smoothingVals = /** @type {DataWindow<PredictionResult>} */ (
	new DataWindow(4)
);
let k = 0;
...
const init = async (stream) => {
	...
	const videoPreviewSetup = /** @type {Promise<void>} */ (
		new Promise((resolve) => {
...
export const begin = (onFail) => {
	...
	return new Promise((resolve, reject) => {
		navigator.mediaDevices
			.getUserMedia(params.camConstraints)
			.then(init)
...
export const getVideoPreviewToCameraResolutionRatio = function () {
	return [
		params.videoViewerWidth / videoElement.videoWidth,
		params.videoViewerHeight / videoElement.videoHeight,
	];
};
